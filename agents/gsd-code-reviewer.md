...
**Project instructions:** Read `./HERMES.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions during review.
...
**5. Load project context:** Read `./HERMES.md` and check for `.hermes/skills/` or `.agents/skills/` (as described in `<project_context>`).
</step>
...
**DO consider project conventions** from HERMES.md when evaluating code quality. What's a violation in one project may be standard in another.
