...
date: 2026-04-12
type: transcript
tags: [agent, memory, langchain, harness, blog-post]
sources:
...
**Original Tweet**: https://x.com/hwchase17/status/2042978500567609738
**Stats**: 231 retweets, 1846 likes, 57 replies
...
As soon as there is any state associated, it's much harder to switch. Because this memory matters. And if you switch, you lose access to it.
...
1. **Agent harnesses = dominant pattern** — không phải models, mà là harnesses (LangGraph, etc)
2. **Harness tied to memory** — short-term (conversation), long-term (cross-session) đều do harness quản lý
3. **Closed harness = no memory ownership** — dùng Claude Agent SDK = memory nằm ở Anthropic
4. **Model providers đang build lock-in qua memory** — không chỉ qua model
5. **Memory is early** — MVP thường chưa có long-term memory
6. **Proprietary dataset** — user interactions + preferences = competitive advantage
7. **Stateless → stateful = harder to switch** — memory tạo lock-in thực sự
