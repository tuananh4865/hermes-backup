...
def _default_spawn(
    ...
    *,
    ...
    # SKILL-BASED DISPATCH DISABLED — kanban lifecycle is fully injected via
    # KANBAN_GUIDANCE; adding --skills kanban-worker here was causing workers to
    # exit with "Unknown skill(s): kanban-worker" despite skill being present at
    # both ~/.hermes/skills/devops/kanban-worker/ and
    # ~/.hermes/profiles/<profile>/skills/devops/kanban-worker/.  The flag was
    # supplementary (pattern library only); removing it is safe.  Task authors
    # can still request per-task skills via task.skills below.
    # if _kanban_worker_skill_available(env.get("HERMES_HOME") or profile_skills_home):
