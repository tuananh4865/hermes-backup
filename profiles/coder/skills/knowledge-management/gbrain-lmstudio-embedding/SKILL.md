...
### Model Disappearance Issue (VRAM pressure)
...
1. Check: `curl localhost:1234/v1/models`
2. If missing, the server dropped it due to memory
3. To unload all models: `curl -X POST http://localhost:1234/v1/unload -H "Content-Type: application/json" -d '{}'`
4. Restart LM Studio app to force clean state

### Tested Models (2026-05-06)

| Model | Format | Size | Status | Notes |
|-------|--------|------|--------|-------|
...
### ByteRover + LM Studio Quick Setup
```bash
