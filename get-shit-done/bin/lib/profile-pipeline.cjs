 ...
 *
 * Reads Hermes Agent session history (read-only) to extract user messages
...
async function cmdScanSessions(overridePath, options, raw) {
  ...
  if (!sessionsDir) {
    const searchedPath = overridePath || '~/.hermes/projects';
    error(`No Hermes Agent sessions found at ${searchedPath}.${overridePath ? '' : ' Is Hermes Agent installed?'}`);
  }
