...
# Hermes Agent X Research — 2026-05-14

## Overview
- **Total mentions found:** 50+
- **Date range:** Last 7 days (May 7-14, 2026)
- **Sentiment:** Highly positive with one notable stability concern
...
| Metric | Value |
|--------|-------|
| Stars | **148.5K+** (up from 140K on May 10) |
| Contributors | 390+ |
| Forks | 23,393 |
| Latest Release | **v0.13.0 (v2026.5.7)** — "The Tenacity Release" — May 7, 2026 |

**v0.13.0 Since v0.12.0:** 864 commits · 588 merged PRs · 829 files changed · 128,366 insertions · 282 issues closed (13 P0, 36 P1) · 295 contributors

---

## v0.13.0 — The Tenacity Release (Major)

**Core theme:** "Hermes now finishes what it starts"

### Key Features:
1. **Kanban Multi-Agent (Durable):** Heartbeat, reclaim, zombie detection, auto-block on incomplete exit, per-task retries, hallucination recovery
2. **`/goal` Command:** Keeps agent locked on a target across turns (Ralph loop)
3. **Checkpoints v2:** State persistence with real pruning
4. **Gateway Auto-Resume:** Interrupted sessions resume after restart
5. **Cron `no_agent` Watchdog Mode:** Background monitoring
6. **8 P0 Security Fixes:** Redaction ON by default, Discord role-allowlists guild-scoped, WhatsApp rejects strangers by default, TOCTOU windows closed
7. **Google Chat:** 20th messaging platform
8. **7 i18n Locales:** Shipped

---

## ⚠️ Stability Concern — v0.13.0

**@davideuler** reported May 9: v0.13.0 scored **0.4/10** on Agent Watch dashboard (release stability tracker). Issue labeled P3, `type/docs`.

> "The new version Hermes Agent v0.13.0 (2026.5.7) seem fairly unstable"

**Community response:** Issue is open, no fix released yet. Consider staying on v0.12.0 if stability is critical.

---

## Top Use Cases (by frequency)

1. **Parallel Testing + Memory Verification** — "Just a parallel test. Hermes Agent is open-source from Nous Research. Every agent tool claims some version of 'memory'..." (@Saboo_Shubham_, Apr 29)

2. **Multi-Agent Workflows via Kanban** — v0.12.0 Kanban board with agents claiming tasks, working in parallel, handing off (@NousResearch, May 3)

3. **Local LLM Routing** — "MiniMax M2.7 routing (10B active, fast), Qwen 27B sub-agents" — interesting pattern: Hermes orchestrator + sub-agents on different local LLMs (@ti_guo_, Apr 21)

4. **PM Workflows on VPS** — £4 VPS setup for recurring competitive monitoring, self-writing skills that compound (@mohit15856, Apr 30)

5. **Self-Writing Skills** — "I set up a competitive monitoring workflow on a Tuesday. Same prompt every Monday for six weeks. By week six, the briefing was surfacing competitor patterns I hadn't caught in three weeks of doing it manually." (@news.aakashg, May 1)

---

## New Techniques/Tips Found

1. **Multi-Provider Routing Pattern** — Use MiniMax M2.7 as orchestrator (fast, 10B active), Qwen 27B for sub-agents — different models for different roles

2. **Agent Isolation per Account** — "Each Hermes agent gets its own accounts (Gmail or agent mail), not yours. Each agent gets its own API keys, scoped..." (@nateherk, "4 days ago")

3. **Quick Setup (< 1 Hour)** — "Hermes Agent looks like a tool for hardcore devs, but you can set it up in less than 1 hour even if you have never touched a terminal." (@Teknium, Mar 31)

4. **Self-Writing Skill Compounding** — Skills rewrite themselves 4x over 6 weeks. Same task: 20 min week 1 → 8 min week 6

---

## OpenClaw vs Hermes Comparison

**Key insight from @screenshotone:** "Hermes Agent is a self-improving agent runtime, while OpenClaw is a gateway-first assistant platform."

| Aspect | Hermes | OpenClaw |
|--------|--------|----------|
| Architecture | Agent-first (learns from work) | Gateway-first (ecosystem reach) |
| Memory | Cross-session, auto-generated skills | Config-file based |
| Self-improvement | Yes, autonomous | Via marketplace (5,700+ skills) |
| Skill creation | Auto from 5+ tool calls | Manual/marketplace |
| Security model | Conservative | More permissive |

**Reddit consensus:** "Hermes is designed as infrastructure for running agent workflows. OpenClaw is designed as a personal AI companion that grows with you."

---

## Frustrations/Issues Reported

1. **v0.13.0 Stability** — 0.4/10 on Agent Watch, P3 issue open
2. **Self-Evaluation Bias** — Agent approves its own skills, may overwrite good manual edits (@bswen, Apr 7)
3. **Setup Complexity for Non-Devs** — Despite claims of 1-hour setup, some users find terminal required

---

## Action Items for Hermes

1. **Monitor v0.13.0 stability** — Watch for patch release addressing the 0.4/10 score
2. **Test local LLM routing** — MiniMax M2.7 + Qwen 27B pattern could reduce costs
3. **Apply PM workflow** — Self-writing skills for recurring tasks (competitive monitoring, daily reports)
4. **Security hardening** — Redaction ON by default is good; verify WhatsApp stranger rejection enabled
...
| Account | Relevance |
|---------|-----------|
| @teknium1 | Core dev, releases |
| @NousResearch | Official announcements |
| @nateherk | Course creator, Hermes deep dives |
| @Gregisenberg | Setup tutorials |
| @Teknium | Getting started guides |
...
| Author | Date | Content |
|--------|------|---------|
| @NousResearch | May 3 | "Hermes Agent now has multi-agent via the Kanban, new in v0.12.0..." |
| @vmiss33 | May 3 | "100% human generated. Includes what I use Hermes agent for..." |
| @nateherk | 4 days ago | "Hermes actually is the real deal. I just dropped a one hour course..." |
| @Saboo_Shubham_ | Apr 29 | "Just a parallel test. Hermes Agent is open-source from Nous Research..." |
| @gregisenberg | Apr 20 | "how to set up hermes agent step by step..." |
| @ti_guo_ | Apr 21 | "MiniMax M2.7 routing (10B active, fast), Qwen 27B sub-agents..." |
| @willknight | Apr 30 | "I'm using GPT-5.5 in Hermes Agent from @NousResearch..." |
| @Teknium | Mar 31 | "Useful guide for getting started with Hermes Agent..." |

---

## Sources
- https://x.com/NousResearch/status/2050997692977844324 (Kanban multi-agent)
- https://x.com/nateherk/status/2053308985575362681 (agent isolation)
- https://x.com/Saboo_Shubham_/status/2049541356767576388 (memory test)
- https://github.com/NousResearch/hermes-agent/releases/tag/v2026.5.7 (v0.13.0 release)
- https://github.com/NousResearch/hermes-agent/issues/22315 (stability concern)
- https://hermes-agent.nousresearch.com/docs/ (official docs)
- https://rumjahn.substack.com/p/complete-guide-to-mastering-hermes (PM guide May 1)
