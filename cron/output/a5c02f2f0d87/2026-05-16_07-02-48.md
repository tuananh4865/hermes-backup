...
# Hermes Agent X Research — 2026-05-16

## Overview
- **Total mentions:** 50+ | **GitHub stars:** 150K+ (up from ~140K on May 10)
- **Sentiment:** Strongly positive, notable frustrations on update bugs

## Top Use Cases
1. **Personal Assistant** — SSH + build + deploy landing page, then text when done (@emmagine79)
2. **Dev Workflow** — 12 parallel instances building Hermes itself
3. **Research Agent** — Watches AI/agent space, delivers daily via Discord/Slack/Notion
4. **Multi-Agent Trading** — Weather bot + Polymarket with 4-layer monitoring
5. **Marketing** — Scrapes landing pages, pulls Meta Ads + TikTok Creative Center, writes briefs in ~4 min
6. **Family Use** — 3 members sharing one $200 ChatGPT sub via WhatsApp

## v0.13.0 "Tenacity Release" (May 7)
- Kanban multi-agent board (heartbeat, zombie detection, hallucination recovery)
- `/goal` keeps agent locked on target across turns
...
## Critical Bugs (Open May 11-16)

| Bug | Issue | Problem |
|-----|-------|---------|
| Token explosion | #24012 | Tool fails → tries unverified alternatives → burns $50+ |
| Update destroys system | #22151 | Can't reverse, commands broken |
| Telegram broken after update | #23783 | `hermes update` doesn't stop gateway |
| Context overflow on model switch | #23767 | Compression INCREASES prompt size (64K→71K) |
| 16K tokens for "who u?" | #13983 | Default bloat |

## Action Items
1. Tool failure → inject "STOP. Search skills first"
2. `hermes update` must stop gateway first
3. Context window guard on model switch
4. Compression invariant: if compressed ≥ original, stop

**Workers status:** ⚠️ Content Creator dead — last output May 11 (5 days ago)
