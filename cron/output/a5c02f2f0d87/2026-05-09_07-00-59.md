...
# Hermes Agent X Research — 2026-05-09

## Overview
- **Total mentions found:** 50+
- **Date range:** Last 7 days (May 2-9, 2026)
- **Sentiment:** Strongly positive
- **Biggest news:** v0.13.0 "Tenacity Release" dropped May 7 — largest release yet (864 commits, 588 PRs)

---

## 🔥 Top Use Cases (by frequency)

1. **Personal automation** — "make 3000$ with AI agent", salary doubling plans, research automation
2. **Multi-agent workflow orchestration** — Kanban boards, parallel task execution, worker coordination
3. **Self-improving assistant** — Memory-driven learning, skill creation from experience
4. **Setup guides & tutorials** — "set up in less than 1 hour", step-by-step configs
5. **Developer tool** — Code tasks, API integrations, self-hosting

---

## 🆕 New Features Found — v0.13.0 Tenacity Release

**Released May 7, 2026** — biggest release yet:
- **Durable Kanban multi-agent board** — heartbeat, reclaim, zombie detection, auto-block on incomplete exit, per-task retries
- **`/goal` persistent command** — locks agent on target across turns (Ralph loop)
- **Checkpoints v2** — real pruning for state persistence
- **Gateway auto-resume** — interrupted sessions auto-resume after restart
- **Cron watchdog mode** — `no_agent` watchdog
- **8 P0 security fixes** — redaction ON by default, Discord role-allowlists guild-scoped, WhatsApp rejects strangers
- **Video analyze tool** — new capability added
...
1. **"Kanban multi-agent pattern"** — agents claim tasks from board, work in parallel, hand off when done
2. **"/goal keeps agent locked"** — persistent goal survives session context changes
3. **Checkpoint v2 with pruning** — state persistence without bloat
4. **Heartbeat + zombie detection** — prevents stale/dead agent processes
5. **Auto-resume interrupted sessions** — gateway recovers from restarts

---

## 💼 Frustrations/Issues Reported

- **Provider auth complexity** — "hardest part is provider authentication" (19 first-class providers, 3 auth paths)
- **Memory claims skepticism** — "Every agent tool claims 'memory' but does it really work?"
- **Setup barriers** — still seen as "for hardcore devs" despite claims of <1 hour setup
- **GPT-5.5 instability** — "goblins and gremlins are with me" with GPT-5.5 in Hermes
...
| vs | What people say |
|----|-----------------|
| **OpenClaw** | Hermes emerges as "faster open-source rival" — lighter, more reliable task execution |
| **Claude Code** | Different use cases — Hermes for persistence/memory, Claude Code for pure coding |
| **Agent Zero** | Docker setup gives "full AI agent army with one command" |
| **GPT-5.5** | Instability issues — "goblins and gremlins" |

---

## 📋 Action Items for Hermes

1. **Update to v0.13.0 immediately** — major stability + security improvements
2. **Explore Kanban multi-agent** — our orchestrator pattern aligns with this
3. **Test `/goal` for long-running tasks** — could improve task completion
4. **Verify checkpoint v2** — better state persistence for our session continuity
5. **Security: confirm redaction ON** — sensitive data protection
...
|---------|--------------|
| @NousResearch | Official — releases, announcements |
| @Teknium | Prominent AI researcher — setup guides |
| @gregisenberg | AI tooling influencer — setup tutorials |
| @AlphaSignalAI | AI OS watch — tracks Hermes releases |
| @willknight | AI journalist — testing advanced configs |
...
| Source | Content | Date |
|--------|---------|------|
| [NousResearch v0.13.0](https://x.com/NousResearch/status/2052493732205744303) | 691 likes — "Tenacity Release" | 1 day ago |
| [Multi-agent Kanban](https://x.com/NousResearch/status/2050997692977844324) | Kanban ships in v0.12.0 | 5 days ago |
| [Setup guide](https://x.com/gregisenberg/status/2046310954351358378) | "built-in memory, 40+ tools" | Apr 20 |
| [Getting started](https://x.com/Teknium/status/2039102514508058675) | "less than 1 hour even if never touched terminal" | Mar 31 |
| [3000$ case study](https://www.youtube.com/watch?v=PLmpcYTuUtI) | "How i made 3000$ with Hermes" | 7 days ago |
| [vmiss33 thread](https://x.com/vmiss33/status/2050984822168830302) | "100% human generated" use cases | 5 days ago |
| [Community](https://x.com/i/communities/2036289138844942498) | 6,605 members discussing setup/help | ongoing |
| [Skeptical review](https://x.com/Saboo_Shubham_/status/2049541356767576388) | "parallel test — skeptical of memory claims" | Apr 29 |

---

## Summary for Anh

**Hot take:** Hermes v0.13.0 is a massive release — the Kanban multi-agent + `/goal` persistence align perfectly with our Felix Model architecture. The security fixes (redaction ON by default) are important. Community is growing fast (6.6K in X community). Someone already monetizing Hermes (3000$/month YouTube video).

**Recommended action:** Update Hermes to v0.13.0 (v2026.5.7) when possible. The Kanban durable board could replace our current worker cron pattern with something more robust.
