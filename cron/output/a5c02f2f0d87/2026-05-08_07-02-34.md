...
**Research Analyst morning — ✅ VERIFIED 2026-05-07:** Ran at 8:31AM, produced market research.
...
- `references/self-improving-agents-2026.md` — **AI agent self-improvement techniques** (2026-05-08): 10 techniques documented — Multi-Agent Reflexion, HyperAgents, A2A/MCP/ACP protocols, Trace Learning, Self-Debugging, SWE-RL, Fallback Chain.
...
# Hermes Agent X Research — 2026-05-08

## Overview
- **Total mentions found:** 75+ results across web search, X, Reddit, blogs
- **Date range:** May 2026 (recent releases: v0.12.0 Curator, v0.10.0 Tool Gateway)
- **Sentiment:** Strongly positive (community growing faster than any prior agent framework)
- **GitHub:** 137,499 stars ▲ | 21,170 forks | 380 contributors | Latest: v2026.5.7 (May 7, 2026)

---

## Top Use Cases (by frequency)

1. **Research Agent** — Daily AI/agent space monitoring, briefs, content angles, delivered via Discord/Slack/Notion/Telegram — *23 mentions*
2. **Personal Assistant** — Family WhatsApp bot, meeting notes, follow-up drafts, calendar — *18 mentions*
3. **DevOps/Server Monitoring** — GitHub issue monitoring, deployment tracking, incident response — *15 mentions*
4. **Trading (Polymarket/Weather)** — Multi-layer order book + on-chain monitoring, self-learning strategy — *12 mentions*
5. **Content Pipeline** — Blog posts, cold emails, competitor research, ad creative briefs — *11 mentions*
6. **Video Generation** — Browser-use + Seedance 2.0 for autonomous movie creation — *5 mentions*

---

## New Techniques/Tips Found

1. **Curator Skill Cleanup (v0.12.0)** — Automatic skill grading, duplicate consolidation, weak skill pruning on schedule. Reports changes to user. Solves "skill library bloat" problem.

2. **Google Meet Integration (v0.12.0)** — Hermes joins meetings, transcribes, speaks, follows up with notes/action items. Bundled natively.

3. **LM Studio First-Class Provider (v0.12.0)** — Local model inference now tier-1. Previously required workarounds. Combined with MiniMax provider addition.

4. **Faster Cold Start (v0.12.0)** — 57% faster terminal launch. Changes how often users do "quick daily tasks."

5. **Background Process Notifications (v0.8.0)** — No more polling to check if long jobs finished. Hermes notifies when complete.

6. **Activity-Aware Timeouts (v0.8.0)** — Replaces wall-clock timeout with tool-activity monitoring. Long-running tasks no longer killed mid-work.

7. **Live Model Switching (v0.8.0)** — Switch providers mid-session without losing context. Use Claude for complex reasoning, Llama for routine tasks.

8. **Skill Self-Grading Rubric (v0.12.0)** — Better evaluation of which skills actually work. Reduces "confidently using broken skill" problem.
...
| Problem | Solution Mentioned |
|---------|-------------------|
| "Agent forgets by morning" (OpenClaw's main complaint) | Hermes persistent memory + FTS5 cross-session recall |
| YAML/config maze for setup | `hermes setup` wizard, sensible defaults |
| Cron job visibility | Background process notifications in v0.8.0 |
| Skill library bloat after months | Curator release (v0.12.0) — auto-grades and prunes |
| Model vendor lock-in | 200+ models via OpenRouter, live switching mid-session |
| OpenClaw migration friction | `hermes claw migrate` — imports persona, memories, skills, API keys |

---

## Frustrations/Issues Reported

1. **Self-evaluation unreliable** — Hermes sometimes confidently uses skills that have been broken since last week. (Reported across multiple reviews)

2. **Manual skill edits get overwritten** — If you manually edit a skill file, Hermes may rewrite it on next use and lose your changes.

3. **Self-learning disabled by default** — New users expect "grows with you" out of the box. Must explicitly enable `persistent_memory` and `skill_generation` in config. Many negative reviews are from users who never enabled the learning loop.

4. **Local model performance gap** — One user reported 1-2 tokens/s through Hermes vs 45 tokens/s direct to LM Studio. Hermes overhead is real for local inference.

5. **Setup still more tedious than OpenClaw** — Despite improvements, some users find configuration more complex than alternatives.

6. **Zero agent CVEs vs OpenClaw's 9 in 4 days** — Security posture is genuinely strong, but some users note this hasn't been independently audited.

7. **Not a coding copilot** — Interactive IDE work (Cursor, Claude Code) still outperforms Hermes for real-time pair programming.

---

## Competitor Comparisons

### vs OpenClaw (most discussed)
| Dimension | Winner | Notes |
|-----------|--------|-------|
| Setup ease | **Hermes** | Wizard vs YAML maze |
| Memory | **Hermes** | Auto vs manual |
| Learning loop | **Hermes** | Auto skill creation |
| Multi-channel breadth | **OpenClaw** | 50+ vs 14 platforms |
| Skill ecosystem | **OpenClaw** | 5,700+ community vs 643 |
| Security (CVEs) | **Hermes** | 0 vs 9 disclosed |
| Stability | **Debated** | OC has more releases, some broken |
| Best for | Split | OC=orchestration, Hermes=execution |

**Community consensus:** Use both. OpenClaw as planner/orchestrator, Hermes as execution specialist. ACP protocol enables communication.

### vs Claude Code / Cursor
- **Coding quality:** Claude Code wins (deeper terminal integration, better diff views)
- **Memory/learning:** Hermes wins (cross-session recall, skill accumulation)
- **Best pattern:** Claude Code for active coding sessions, Hermes for background/automation

### vs ChatGPT / Claude.ai
- **Persistence:** Hermes wins (local memory files, MIT license)
- **Ease of use:** Commercial products win (no setup)
- **Customizability:** Hermes wins (any model, any endpoint)
...
### 🔥 Investigate/Apply
1. **v0.12.0 Curator release** — Test skill auto-cleanup on Anh's instance. If working, document workflow for skill maintenance.
2. **Google Meet integration** — Enable for Anh's meeting workflow if applicable.
3. **MiniMax provider** — v0.12.0 promoted MiniMax to first-class. Verify Anh's current provider setup.

### ⚠️ Fix Based on Complaints
1. **Self-learning disabled by default** — Check Anh's `~/.hermes/config.yaml` to confirm `persistent_memory` and `skill_generation` are enabled. This is the #1 user complaint.
2. **Skill overwrite issue** — Document how to pin skill files to prevent auto-rewrite. Check if `pinned` frontmatter field is working.
3. **Verify memory is actually persisting** — Many "broken memory" reports are actually users who never enabled the feature. QA check on Anh's setup.

### 💡 New Features to Consider
1. **Per-skill usage analytics** — Track which skills get called, how often, success rate. Feed into Curator grading.
2. **Skill diff view** — Show what Hermes changed in a skill after auto-rewrite, so manual edits aren't silently lost.
3. **Git integration for skills** — Version control skill files so overwrite doesn't mean data loss.
...
|---------|--------------|
| [@Teknium](https://x.com/teknium) | Nous Research co-founder, runs 12 Hermes instances in parallel to build Hermes Agent |
| [@gkisokay](https://x.com/gkisokay) | Research agent workflow — daily AI monitoring, multi-platform delivery |
| [@alexcovo_eth](https://x.com/alexcovo_eth) | Video generation via Hermes + Browser-use + Seedance 2.0 |
| [@adiix_official](https://x.com/adiix_official) | Polymarket trading with 4-layer Hermes monitoring |
| [@EXM7777](https://x.com/EXM7777) | Family WhatsApp bot, 3 members, "proactive behaviors" |
| [@mvanhorn](https://x.com/mvanhorn) | Business ops automation, client research, content pipeline |
| Robert Scoble | Quoted user saying "spooked me with detail from 3 weeks ago" |
...
| Metric | Value |
|--------|-------|
| GitHub Stars | 137,499 (top 100 all-time) |
| Growth | Fastest-growing agent framework 2026 |
| Community | ~30K r/hermesagent members (started March 14, 2026) |
| Latest Release | v2026.5.7 (May 7, 2026) |
| Release Velocity | 12 releases since July 2025 |
| Skills Hub | 643+ community skills on agentskills.io |
| Platforms | 14+ messaging platforms |
| Providers | 20+ LLM providers (OpenRouter, Claude, GPT, MiniMax, etc.) |
| Contributors | 380 |
| Pricing | MIT (free), $5 VPS deployment |

---

## Raw Data (Top 25 Links)

1. https://github.com/nousresearch/hermes-agent — Main repo (137K stars)
2. https://hermes-agent.nousresearch.com/ — Official site
3. https://hermes-agent.nousresearch.com/docs/user-stories — Official user stories
4. https://hermesatlas.com/reports/state-of-hermes-april-2026 — State of Hermes (April 2026)
5. https://hybrid-llm.com/workflow/ai-agents/hermes-agent-user-stories/ — Real user stories analysis
6. https://dreamsaicanbuy.com/blog/why-hermes-is-better-than-openclaw — Migration story
7. https://kilo.ai/articles/openclaw-vs-hermes-what-reddit-says — Reddit synthesis (25 threads, 1300+ comments)
8. https://georgelarson.me/writing/2026-03-19-hermes-review/ — Honest review
9. https://www.heyuan110.com/posts/ai/2026-04-14-hermes-agent-guide/ — Hands-on review v0.9
10. https://juliangoldie.com/hermes-agent-v0-12-0-update/ — v0.12.0 Curator update
11. https://juliangoldie.com/hermes-agent-v0-8-0/ — v0.8.0 reliability update
12. https://www.youtube.com/watch?v=r5FrGDTCYSg — Charlie Chang tutorial (2026-05-02)
13. https://www.youtube.com/watch?v=6SWLAzU6VAs — 7 real use cases (2026-04-21)
14. https://plainai.tech/articles/hermes-agent-nous-research-fastest-growing-github-use-cases — Growth analysis
15. https://pick-right.com/tools/hermes-agent/ — Review with security comparison
16. https://dev.to/truongpx396/hermes-agent-deep-dive-build-your-own-guide-1pcc — Deep dive (April 30)
17. https://medium.com/@sathishkraju/i-switched-from-openclaw-to-hermes-agent-heres-what-nobody-told-me-5f33a746b6ca — Migration guide
18. https://news.bitcoin.com/what-is-hermes-agent-nous-researchs-self-improving-ai-explained/ — Explainerm
19. https://espressio.ai/blog/hermes-agent-guide-2026/ — Complete guide
20. https://hostinger.com/tutorials/hermes-agent-use-cases — 10 use cases
21. https://openaitoolshub.org/en/blog/hermes-agent-ai-review — Review (March 16)
22. https://hermesagent.org.cn/en/ — Chinese docs
23. https://hermes-agent.us/ — Deploy guide
24. https://hermesagent.best/ — Community site
25. https://github.com/nousresearch/hermes-agent/releases — Release history

---

## Summary for Anh

**Hermes Agent is on a tear.** 137K GitHub stars in ~10 months, fastest-growing agent framework of 2026. The v0.12.0 "Curator" release just shipped (May 4) with automatic skill cleanup — solving the main long-term complaint about skill library bloat.

**What matters for Anh's setup:**
1. **Check if self-learning is actually enabled** in config — this is the #1 issue users complain about
2. **v0.12.0 skill Curator** — test it on Anh's skill library
3. **MiniMax provider is now first-class** in v0.12.0 — could simplify provider config
4. **Community is explosive** — 30K subreddit members, 643 skills, active Discord

**Competitive position:** Hermes wins on learning/memory vs OpenClaw, but OpenClaw still wins on integration breadth. Community converging on "use both" pattern — Hermes for execution + learning, OpenClaw for orchestration. This aligns with Anh's Felix Model vision.
