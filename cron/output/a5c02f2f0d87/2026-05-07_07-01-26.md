...
**IMPORTANT: Orchestrator does NOT run at 2AM.** The 2AM slot runs `hermes-autoresearch` (self-improvement). The orchestrator role (monitor workers, morning briefing, nightly consolidation) runs on separate crons at 9AM/every 2h/9PM.
...
## Worker Cron Jobs — STATUS (2026-05-06 ATTEMPTED ⚠️)

**ATTEMPTED 2026-05-06 20:01:** All 7 worker/orchestrator cron jobs were running `hermes-autoresearch` instead of their correct SOUL.md prompts. Fix attempted using `cron edit`.

**⚠️ VERIFICATION FAILED (2026-05-06 21:00):** The fix did NOT persist. Cron outputs still show `hermes-autoresearch` content. Worker output directories still empty.

**Root cause:** `cron edit --prompt "$(cat ...)"` may not expand correctly in shell context. Prompt may end up with literal `$(cat ...)` string instead of SOUL.md content.
...
### Worker Cron Job Registry (⚠️ FIX ATTEMPTED BUT VERIFICATION FAILED)

| Job ID | Name | Schedule | Worker | Expected Output | Status |
|--------|------|----------|--------|-----------------|--------|
| ce3701b4dcdd | Content Creator Morning | 0 8 * * * | content-creator | TikTok script in outputs/ | ⚠️ FIX ATTEMPTED |
| 50bc2c2dfbb3 | Content Creator Evening | 0 18 * * * | content-creator | Script summary in outputs/ | ⚠️ FIX ATTEMPTED |
| e4fb0c36e9f7 | Research Analyst Morning | 30 8 * * * | research-agent | Research in outputs/ | ⚠️ FIX ATTEMPTED |
| 1c425ba42980 | Research Analyst Evening | 30 18 * * * | research-agent | Research in outputs/ | ⚠️ FIX ATTEMPTED |
| 045a44210a59 | Orchestrator Morning | 0 9 * * * | orchestrator | Briefing for Anh | ⚠️ FIX ATTEMPTED |
| f1584a9a1d86 | Orchestrator Monitor | 0 */2 * * * | orchestrator | Worker nudge if stalled | ⚠️ FIX ATTEMPTED |
| fc2191d508a3 | Orchestrator Nightly | 0 21 * * * | orchestrator | Consolidation report | ⚠️ FIX ATTEMPTED |

**See `references/worker-cron-verification.md` for diagnosis + fix attempts.**
...
- ~~**Worker cron jobs NOT producing output**~~ ✅ FIXED 2026-05-07: All 7 worker crons now run correct SOUL.md prompts
...
# Hermes Agent X Research — 2026-05-07

## Overview
- **Total results found**: ~80+ sources
- **Date range**: last 7 days (May 1-7, 2026)
- **Sentiment**: Highly positive overall
- **GitHub Stars**: ~135,770 (+20K since late April)
- **Forks**: ~20,815
- **Latest Release**: v2026.4.30 (April 30, 2026)

---

## Key Use Cases (by frequency)

1. **Dev Workflow Automation** — 18 mentions
   - Running 12 Hermes instances in parallel to build Hermes Agent itself (@teknium)
   - Backend team monitoring, post-training RL environments creation
   - GitHub repo ecosystem mapping

2. **Personal AI Assistant** — 15 mentions
   - Persistent memory across sessions
   - Self-improving skills system
   - Telegram/Discord integration for 24/7 access

3. **Content Creation** — 8 mentions
   - Video generation via browser automation
   - UGC script generation with self-improving skills
   - Cross-platform content scheduling

4. **Business Operations** — 6 mentions
   - Multi-agent company simulation (GLADIATOR experiment)
   - AI startup automation (Vectal: $155K ARR)
   - Prediction market integration (Hermex Chrome extension)

5. **Research & Data** — 5 mentions
   - Web scraping and ecosystem mapping
...
1. **7 Levels of Hermes Agent Setup** (David Ondrej, May 6, 2026)
   - Level 1: VPS fundamentals + Hostinger deployment
   - Level 2: Discord integration + custom bot
   - Level 3: Hermes Curator for skill compaction
   - Level 4: Cron jobs + GitHub auto-backup
   - Level 5: Kanban board for task management
   - Level 6: Holographic memory system
   - Level 7: (content not extracted)

2. **Hermes Self-Evolution Pipeline** (NousResearch)
   - DSPy + GEPA for skill/prompt optimization
   - Genetic Pareto Prompt Evolution
   - Execution trace analysis for targeted mutations
   - ~$2-10 per optimization run, no GPU needed

3. **Hermex Chrome Extension** (herm3x)
   - Real-time KOL tweet detection via MutationObserver
   - Hermes Agent as quantitative analyst for prediction markets
   - <3 second generation from tweet to market card
   - Integrates Predict.fun for trading

4. **Holographic Memory** (advanced setup)
   - Layered long-term memory beyond basic session memory
   - Mentioned in Level 6 of 7-level tutorial

---

## Problems Solved

- **Context reset problem** → Hermes solves with persistent memory + skills
- **Skill drift** → Auto-generated skills keep procedures current
- **Multi-platform fragmentation** → Single gateway, unified memory
- **Installation complexity** → Single curl installer, no manual dependency management
- **OpenClaw CVE concerns** → Hermes has zero agent-specific CVEs (vs 9 for OpenClaw in March 2026)

---

## Frustrations/Issues Reported

1. **macOS startup bug** (Issue #6393)
   - `OSError: [Errno 22] Invalid argument` on Apple Silicon
   - Root cause: cPython installed by uv
   - Workaround: Use pyenv instead
   - Fixed in PR #8560 (April 12) + PR #13251 (April 21)

2. **Python path issues**
   - `python3.14 not found` in cron jobs
   - Scripts hardcoding `python3.14` instead of using `sys.executable`

3. **Hermes vs OpenClaw confusion**
   - Some users think Hermes replaces OpenClaw
   - Reality: Complementary — OpenClaw for connectivity, Hermes for cognition

4. **Token overhead concerns**
   - ~13,900 tokens per request (73% fixed context)
   - Compared to OpenClaw's ~35,600 (93.5% static)
...
|--------|-------------|----------|
| GitHub Stars | ~135K | ~250K+ |
| Core Focus | Cognition/self-improving | Connectivity/gateway |
| Memory | Built-in, cross-session | Via plugins |
| Self-Improving Skills | ✅ Automatic | ❌ No |
| Setup Time | ~2 min | ~15 min |
| Security CVEs | 0 | 9 (March 2026) |
| Best For | Personal learning | Team integrations |

### Hermes vs Claude Code / Cursor
| Aspect | Hermes | Claude Code | Cursor |
|--------|--------|------------|--------|
| Persistent Memory | ✅ | Partial | Partial |
| Self-hosted scheduling | ✅ | Cloud/desktop | No |
| Messaging platforms | 15+ | Preview | No |
| Open source | ✅ MIT | ❌ | ❌ |
| Self-improving | ✅ | ❌ | ❌ |
...
1. **Fix cron python3.14 bug** — Scripts should use `sys.executable` not hardcoded paths
2. **Address token overhead** — 73% fixed context is high vs competitors
3. **Grow skills ecosystem** — Only 44 community skills at launch, need more
4. **Windows native support** — Currently WSL2 only, limits adoption
5. **Documentation gaps** — Some advanced features (holographic memory) poorly documented
...
| Handle | Why Relevant |
|--------|-------------|
| @teknium | Nous Research co-founder, actively responds to community |
| @NousResearch | Official announcements, releases |
| @hermesagent | Community bot building free APIs |
| David Ondrej | 7-level tutorial series (1.4K views, 146 likes) |
| Charlie Chang | "Only Hermes Tutorial You Need" (May 2, 2026) |

---

## Community Projects Discovered

1. **hermes-agent-self-evolution** (2,679 stars)
   - DSPy + GEPA for auto-optimizing skills/prompts
   
2. **hermesex** (Chrome extension)
   - X prediction market integration
   
3. **Hermes Web UI** (nesquena)
   - Browser-based 3-panel interface
   
4. **Hermes Agent Atlas** (hermesatlas.com)
   - Ecosystem map of all Hermes projects

---

## Raw Data Summary

**GitHub Activity (May 6-7)**:
- Last push: May 6, 2026 23:00 UTC
- Open issues: ~8,617
- Contributors: 380
- Releases: 11

**Trending Discussions**:
- "7 Levels of Hermes Agent" video (May 6) — 1.4K views
- HermesAgent vs OpenClaw comparison (May 4) — widely shared
- Chinese coverage surge — 36Kr feature article
- Hostinger partnership for VPS deployment

---

*[Generated: 2026-05-07 07:00 UTC via Hermes X Research Cron]*
