...
# Hermes Agent X Research — June 15, 2026

## Overview
- **Total mentions found:** 50+
- **Date range:** Last 7-14 days (late May – June 15, 2026)
- **Sentiment:** Positive (predominant) / Neutral (issues reported)
- **GitHub Stars:** ~189K (up from 176K on June 10 — significant growth)

## Major Developments This Week

### Hermes v0.16 "The Surface Release" (June 5, 2026)
- **874 commits · 542 PRs · 170 contributors**
- **Native Desktop App** — macOS, Windows, Linux public preview
  - Built across 100 PRs + 159 commits in ONE WEEK
  - One-click install, in-app self-update
  - Drag-drop files, clipboard paste, Cmd+K palette
...
- **/undo** command — take back last N turns
- **Fuzzy model picker** everywhere
- **NVIDIA/skills** as trusted skills tap (CUDA-X, AIQ, cuOpt)
- Release announcement: 296 likes on X

### ⚠️ Breaking Change — May 28 Update
- **Issue:** Latest update (May 28) broke container deployment — won't start
- **Workaround:** Reverted to `nousresearch/hermes-agent:v2026.5.16`
- **Status:** Reported on Reddit May 28, fix committed in subsequent releases

### xAI + X Premium Integration (May 16)
- X Premium+ subscribers can use Grok via Hermes
- Hermes Agent can now search X posts natively
- Setup guide published by xAI team (May 19)

## Top Use Cases (by frequency)

1. **Dev Workflow Automation** — 61 community-documented use cases
   - Remembers repo conventions, test commands, branch naming
   - Prioritizes failed approaches, knows which PRs to review

2. **Multi-Platform Integration** — 65 use cases
   - Telegram, Discord, Slack, WhatsApp, Signal, Email, CLI
   - Desktop app extends to native cross-platform

3. **Self-Improving Skill Creation**
   - Skills created autonomously, refined during use, compounded over time
   - vs OpenClaw: OpenClaw skills are static files you maintain manually

4. **Business Automation**
   - Found mispriced supercars (autonomous research)
   - Replaced $497/month automation tools
   - Research + content pipelines

5. **Memory-Powered Coordination**
   - Persistent coordinator across sessions
   - Remembers user preferences, workflows, lessons learned

## Comparison with OpenClaw (Community Consensus)

| Dimension | Hermes Agent | OpenClaw |
|-----------|-------------|----------|
| **Self-Improvement** | ✅ Learns from tasks, compounds over time | ❌ Static skills |
| **Security** | ✅ Zero agent CVEs (as of May 2026) | Lower priority |
| **Cost** | $5/mo serverless hosting | Higher |
| **Control** | Safer-by-default, long-running | Tighter manual control |
| **Skills** | Dynamic, auto-created | Static files |
| **Best For** | Autonomous agents that grow | Control-plane focus |

**Key quote (Reddit):** "OpenClaw is a better control plane. Hermes is a better self-improving runtime."

## Frustrations/Issues Reported

1. **Tools not working** — Requires better model or API keys; Skill Seeker suggested as fix
2. **Not a "set-and-forget" solution** — Requires babysitting (June 1 post)
3. **Self-evaluation unreliability** — Agent occasionally reports success when outcome failed
4. **May 28 breaking update** — Container deployment broken, workaround needed
5. **Windows compatibility issues** — Runtime detection reports Scheduled Task errors (Issue #25502)
6. **Setup complexity** — Requires more initial configuration than OpenClaw

## Action Items for Hermes

- [ ] Investigate May 28 breaking change — ensure v0.16.0+ doesn't regress container deployment
- [ ] Address self-evaluation reliability — top complaint in comparisons
- [ ] Document "not set-and-forget" expectation in onboarding
- [ ] Continue Windows compatibility fixes

## Notable People/Accounts

| Account | Relevance |
|---------|-----------|
| @NousResearch | Official announcements, releases |
| @xai | xurl skill setup, X Premium integration |
| @Teknium | Hermes founder — runs 12 Hermes instances for Hermes development |
| @hermesagent X Community | 7.7K members, active discussions |

## Gen Z Slang — No New Terms

**Status:** Web search finds only already-documented terms (Đỉnh, Toang, Gato, SÍT RỊT, KHÓ QUÁ BỎ QUA, etc.)

**Known terms (updated June 10):**
- Thả thính — lighthearted flirting
- Lầy — being humorously ridiculous
- Phê — feeling thrilled/excited

**Note:** Workers permanently deleted May 25 — slang sync via worker output no longer possible.

## Raw Data Sources

- NousResearch X: https://x.com/NousResearch/status/2063075373785428083 (v0.16 release, 296 likes)
...
- Reddit Megathread: https://www.reddit.com/r/hermesagent/comments/1t6gf4j/
- YouTube Comparison: https://www.youtube.com/watch?v=EQNfEuCatXE (May 30)
- Breaking bug report: https://www.reddit.com/r/hermesagent/comments/1tqbatf/

---

**Status:** ✅ Research complete. Commit b974cc6d3. Wiki clean, SHS=0, 189K stars.
