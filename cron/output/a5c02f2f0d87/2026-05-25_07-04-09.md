...
For promising URLs, extract with `web_extract`. If 400/403, use browser as fallback.
...
**⚠️ WORKER DEATH ESCALATION (2026-05-22):** Workers now COMPLETELY DEAD — output directories EMPTY (no files at all), not just stale. Directories exist but `ls` returns nothing. This is beyond "stale" — the cron jobs themselves have stopped firing. Must use web search fallback AND report worker restart need to Anh.

**TONIGHT CONFIRMED (2026-05-24):** Complete write-off. Workers still dead, web search also down. See `references/autoresearch-2026-05-24-complete-failure.md` for full details.

**Worker death escalation path:**
...
# Hermes Agent X Research — 2026-05-25

## Overview
- **Total mentions:** 50+ across X, GitHub, Reddit, LinkedIn, YouTube
- **Date range:** May 18-25, 2026
- **Sentiment:** Strongly positive, accelerating growth

## Latest Metrics

| Metric | Value | Change |
|--------|-------|--------|
| GitHub Stars | **157.2K+** | ↑2K from May 20 |
| Global Rank | **#46** | ↑ from #48 |
| Weekly Growth | +869 stars | 345 pushes |
| Contributors | 1.3K+ | Growing |

**🚀 Major milestone (May 10):** Hermes Agent **overtook OpenClaw** as most-used open-source AI agent on OpenRouter's global daily inference rankings.

## v0.14.0 "Foundation Release" — May 16

**Theme:** "Installs and runs anywhere, ships with what you actually want."

| Feature | Description |
|---------|-------------|
| Native Windows Beta | Full PowerShell installer, no WSL2 required |
| 180x faster browser | CDP-based rewrite |
| Live session handoffs | `/handoff` command for context transfer |
| Local OpenAI proxy | Easier LM Studio / ollama integration |
| LINE + SimpleX Chat | New messaging platforms |
| Microsoft Teams | Pipeline + webhook adapter |
| Vision integrations | Image understanding built in |

**Community reaction:** "Hermes V0.14 is INSANE!" — 691 likes on release announcement.

**Top requests:** Swarm agent tutorials, kanban, conductor, workspace UI documentation.

## X Premium Integration — May 16

@NousResearch: "You can now use X Premium subscriptions in Hermes Agent, and Hermes Agent can now search X posts. Connect Grok to Hermes Agent."

## Nvidia Partnership — May 13

**"Hermes Unlocks Self-Improving AI Agents, Powered by NVIDIA RTX"** — Blog on NVIDIA RTX AI Garage. Hermes crossed 140K stars in under 3 months, "most used agent in the world" as of last week.

## Desktop App — May 10

"There's now a Hermes Agent Desktop App" — Run self-evolving 24/7 autonomous AI agent in full native desktop UI, multi-agent support via desktop interface.

## Top Use Cases

1. **Multi-Agent Teams (Production)** — Teknium runs **12 Hermes instances in parallel**:
   - Backend team monitors stack issues
   - Post-training team creates RL environments
   - Dataset investigation and manipulation
   - Auto-Build: plan → code → QA → ship

2. **Business Automation** — 24/7 research pipelines, content creation, customer comms

3. **Personal AI Assistant** — Cross-session memory, builds reusable skills from experience

4. **Self-Evolving Skills Marketplace** — hermes-skill-marketplace (13 stars) writes/tests/publishes skills autonomously

## Competitor Comparisons

- **vs OpenClaw:** Hermes wins on OpenRouter daily rankings. Stronger memory, learns procedures not just preferences.
- **vs LangGraph:** Hermes is "between CLI tool and chat platform." LangGraph = orchestration graphs. Hermes = autonomous skill creation + memory.

## Frustrations Reported

1. **Learning loop unpredictability** — "babysitting what the agent decides to learn"
2. **Skill quality varies** on complex multi-phase tasks
3. **Documentation gaps** for swarm agents, operations, conductor, kanban

## Notable Accounts
- @NousResearch — Official announcements
- @Teknium — Founder, 12-agent production pattern
- @Scobleizer — Monthly Hermes Report
- @doronkatz — OpenRouter rankings tracker

## Key Insights

1. **Hermes = dominant open-source agent** — overtook OpenClaw on OpenRouter
2. **Platform expansion** — Windows, LINE, SimpleX, Teams = "installs and runs anywhere"
3. **Community-driven** — Swarm agent tutorials + kanban docs are top requests
4. **12-agent production pattern** — Teknium proof-of-concept for multi-agent coordination
5. **Business viability** — Nvidia partnership validates enterprise potential

## Files Created/Updated
- `queries/hermes-x-research-2026-05-25.md` (5,805 bytes)
- `index.md` — Added new entry
- `log.md` — Appended research log
- Git commit: `4a33771`
