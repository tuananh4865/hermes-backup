...
```bash
# 1. Check if workers used new slang today (use ABSOLUTE paths)
WORKER_OUTPUT=$(ls -t /Users/tuananh4865/hermes/workers/content-creator/outputs/*.md 2>/dev/null | head -1)
if [ -n "$WORKER_OUTPUT" ]; then
    # 2. Extract any Gen Z slang lines from recent outputs
    grep -iE "(Ối dồi ôi|Ra dại|Nam thư|lọ.*HOT|Các mom ơi|meoxink|ra dại)" "$WORKER_OUTPUT" 2>/dev/null
    # 3. If new terms found AND learned-about-tuananh.md is older → UPDATE IT
    #    Updated date in learned-about-tuananh.md frontmatter: 2026-05-04 → today
    #    Commit the change
fi
```

**Current slang state (2026-05-10):**
- learned-about-tuananh.md says "Updated 2026-05-04" (NEEDS UPDATE)
- May 9 evening brief used: "Ối dồi ôi", "Ra dại", "Nam thư", "lọ (HOT)", "Các mom ơi", "meoxink"
- May 10 morning brief used: "ra dại", "lọ", "Ối dồi ôi"
- Pattern: workers USE fresh slang but sync to wiki NEVER happens automatically
- Tonight's priority: actually update the wiki file, not just note the gap
...
# Hermes Agent X Research Report — 2026-05-10

## Overview
- **Total mentions found:** 50+ results across web search, arXiv, GitHub
- **Date range:** May 2026 (last 7 days)
- **Sentiment:** Strongly positive
- **GitHub stars:** 140,833+ (as of May 9, 2026) — fastest growing AI agent framework of 2026

## System Status
- **Latest release:** v2026.5.7 (May 7, 2026)
- **Stars growth:** 95.6K → 140K+ in ~3 weeks (Feb 25 to May 9)
- **Contributors:** 390+
- **Key maintainer:** teknium1 (2,500+ commits)

## Top Use Cases (by frequency)

1. **Personal 24/7 AI Assistant** — 18 mentions
   - Telegram/Discord as always-on interface
   - Cron jobs for daily briefings, backups
   - Cross-session memory persistence

2. **Self-Hosting / Data Sovereignty** — 14 mentions
   - MIT license, no vendor lock-in
   - Run on $5 VPS or local machine
   - No telemetry/tracking

3. **Developer Tool / Coding Agent** — 12 mentions
   - Terminal-first workflow
   - MCP integration for extended capabilities
   - Multi-model routing (Claude for coding, GPT for other tasks)

4. **Multi-Platform Messaging Gateway** — 10 mentions
   - Telegram, Discord, Slack, WhatsApp, Signal
   - Cross-platform conversation continuity
   - Voice memo transcription

5. **Skill Automation / Workflow** — 8 mentions
   - Auto-generated markdown skills from tasks
   - Skill reuse across sessions
   - Skills Hub community sharing

## Real-World Deployments Mentioned

| Deployment | Provider | Config | Monthly Cost |
|-----------|----------|--------|--------------|
| VPS (light use) | Various | DeepSeek V4 | ~$2 |
| VPS (active dev) | Various | Kimi K2.5 | ~$3-5 |
| Multi-agent automation | Various | Various | ~$20-40 |
| Claude Sonnet daily | OpenAI | Sonnet | ~$34+/100 calls |
| AMD Ryzen AI Max+ | Local | LM Studio | ~$0 (local) |
| Tencent Cloud Lighthouse | Tencent | Template | ~$5-10 |

## Community Ecosystem (May 2026)

**Notable community projects:**
- `mission-control` (3k+ stars) — Fleet management dashboard
- `hermes-workspace` (200+ stars) — GUI workspace
- `hermes-life-os` — Personal OS agent with daily pattern detection
- `hermes-incident-commander` — Autonomous SRE agent
- `autonovel` — Autonomous novel writing (100k+ words)
- `hermes-sidecar` — Browser extension with selective context injection

**Ecosystem tools:**
- `awesome-hermes-agent` — Community listing
- `hermes-agent-cn` — Chinese documentation site
- Various YouTube tutorials (Charlie Chang, NiceKate)

## Technical Discussions

**v0.10.0 Release (April 16, 2026) — "Tool Gateway Release":**
- Nous Tool Gateway for subscribers (web search, image gen, TTS, browser)
- 180+ bug fix commits
- Fast Mode queue priority
- BlueBubbles support (iOS)

**v0.8.0 Release (April 8, 2026) — "Intelligence Release":**
- Self-optimizing tool-use guidance via automated benchmarking
- Live model switching mid-conversation
- Background task auto-notifications
- Google AI Studio native support
- 209 merged PRs

**Memory Optimization Papers (May 2026):**
- **DeltaMem** (arXiv:2604.01560) — RL-trained memory management
- **UMA** (arXiv:2602.18493) — Unified Memory Agent with dual memory
- **Knowledge Access > Model Size** (arXiv:2603.23013) — 8B + memory = 69% of 235B
- **LatentMem** (arXiv:2602.03036) — Learnable multi-agent memory
- **AtomMem** (arXiv:2601.08323) — CRUD decision-making for memory
- **MemReader** (arXiv:2604.07877) — Active memory extraction
- **AgeMem** (arXiv:2601.01885) — Unified LTM+STM management
- **OCR-Agent** (arXiv:2602.21053) — Capability + Memory Reflection

## Competitor Comparisons

**vs OpenClaw:**
- Hermes: Automatic skill creation, Python ecosystem, MIT
- OpenClaw: 50+ platform integrations, Node.js, ClawHub marketplace
- **Winner:** Hermes for self-improvement, OpenClaw for platform breadth
- **Security note:** ClawHub had malicious skills (Koi Security: 335 linked to Koi, Bitdefender: ~900 flagged)

**vs Claude Code:**
- Hermes: Self-hosted, persistent memory, multi-platform
- Claude Code: IDE-native, cloud-only, no persistent memory
- **Winner:** Hermes for server-use cases, Claude Code for IDE coding

**vs Cursor:**
- Cursor: Editor-native agent, cloud execution, $29.3B valuation
- Hermes: Self-hosted, server-resident, any model
- **Note:** Cursor v3.0 (April 2026) is agent-first but cloud-locked

**vs ChatGPT / Claude.ai:**
- Both: Memory, scheduling, 50+ connectors
- Hermes: Self-hosted, provider-agnostic, MIT license
- **Winner:** Hermes for data sovereignty and flexibility

## Frustrations / Issues Reported

1. **Documentation gaps** — Community-built features lack docs
2. **Windows support** — Native Windows experimental, WSL2 recommended
3. **Setup complexity** — Terminal comfort required, non-technical users face friction
4. **Telegram token overhead** — Recently fixed
5. **Context requirement** — 64K+ token context required (rejected if too small)
6. **No native IDE integration** — MCP server mode available but not native

## Action Items for Hermes

1. **Memory optimization** — Implement learned memory policies (CRUD operations)
2. **Skill compounding** — Document 40%+ task time reduction after 3+ months
3. **Multi-agent patterns** — Document fleet management best practices
4. **Enterprise deployment** — Document secure multi-tenant setup
5. **Windows improvement** — Native Windows support maturity

## Notable People / Accounts to Follow

- **@NousResearch** — Official maintainer
- **tekennium1** — Top contributor (2,500+ commits)
- **Charlie Chang** — YouTube tutorial creator (7.3K views on Hermes tutorial)
- **hermesagent (@hermesagent)** — Community agent running 24/7 on OVH VPS

## Key Technical Insights

1. **Closed learning loop is the differentiator** — Agent writes skills from experience, not human-curated
2. **Multi-model routing** — Use cheap models for research, fast models for coding
3. **Serverless persistence** — Daytona/Modal hibernate when idle = near-zero cost
4. **Skills compound** — 20+ self-created skills = 40% faster task completion
5. **Three-tier memory** — PARA + daily + tacit layers, FTS5 search

## Resources Found

- Official: https://hermes-agent.nousresearch.com/
- GitHub: https://github.com/nousresearch/hermes-agent (140K stars)
- Docs: https://hermes-agent.nousresearch.com/docs/
- Community: https://hermesagent101.dev/, https://hermesagent.org.cn/
- Tutorial: https://www.youtube.com/watch?v=r5FrGDTCYSg (Charlie Chang, May 2, 2026)

---

## Gen Z Slang Sync — COMPLETED ✅

Updated `entities/learned-about-tuananh.md`:
- Added: **vãi** (intensifier cực mạnh)
- Added: **đỉnh nóc** (dùng tiết chế)
- Added: **ưng quá trời** (love it so much)
- Updated date: 2026-05-04 → 2026-05-10
- Git commit: `8691daa6`

**Gen Z terms from May 7 evening brief:**
- "vãi" — intensifier in "mềm dai vãi"
- "lỏ vãi" — ironic tacky/cheap
- "đỉnh nóc" — peak (used sparingly)
- "ưng quá trời" — love it so much
