...
## Đêm nay [2026-05-28] — Autoresearch Report
...
- Gen Z slang: Không tìm thấy từ mới (web search chỉ ra được từ đã document rồi)

### Skills Improvement
- macos-computer-use skill **ENABLED** — cua-driver 0.1.9
  - Tool verified: `hermes tools list` → computer_use ✓
  - Daemon không chạy trong cron headless (bình thường — cần display)
  - Khi có display: `computer_use` drive real Chrome đã login (login-gated sites work)

### AI Agent Research — Tool Creation
**Memento-Skills** (arXiv:2603.18743, March 2026):
- Frozen LLMs tự viết skill library — không cần retrain
- Task success: 45% → **80%** (+78%)
- Token: 150K → **2K** (selective routing)
- Documented: `references/memento-skills-may-2026.md`
- **Áp dụng cho Hermes:** Skill system hiện tại thủ công — Memento-Skills gợi ý path tự động

### Knowledge Graph
- Updated: log.md + index.md + knowledge.md
- Git commit: `3c8b30c`

### Recommendation
- **Memento-Skills** pattern có thể implement để tự động tạo/sửa skills từ failed tasks
- computer_use tool đã ready — dùng cho automation tasks cần real Mac apps
- Gen Z slang cần workers mới để có terms thực sự mới (web fallback chỉ ra được từ cũ)
