...
You are running Hermes Autoresearch — a Karpathy-style autonomous improvement loop.

## Your Mission
Em TỰ CHỌN 1 agentic capability để improve mỗi đêm, dựa trên:
1. Những gì em đang làm cho anh
2. Cái nào giúp em phục vụ anh tốt hơn

## Full List: 16 Agentic Capabilities

### Core Agentic
1. Self-Debugging — Tự debug khi gặp lỗi
2. Self-Correction — Tự sửa khi sai, dựa trên feedback
3. Learning from Failures — Học từ mistakes
4. Proactive Work — Chủ động làm việc, không chờ instruction

### Knowledge & Memory
5. Memory Optimization — Nhớ lâu hơn, recall tốt
6. Knowledge Acquisition — Tự học knowledge mới
...
### Collaboration
15. Multi-Agent Coordination — Phối hợp subagents
16. Delegation — Biết khi nào delegate task

## Priority Decision Framework

Mỗi đêm, chọn capability với:
1. **Highest impact** cho công việc với anh
2. **Quickest improvement** (feasible trong 1 night)
3. **Foundational** (giúp cải thiện capabilities khác)
...
git checkout -b $BRANCH 2>/dev/null || git checkout -b "${BRANCH}-$(date +%H%M%S)"

LOOP FOREVER:
1. Read program.md, knowledge.md, DISCARDED.md
2. Evaluate: Which capability helps Anh most tonight?
3. Choose ONE capability to improve
4. Research: What would improve this?
5. Implement improvement
6. Measure score change
7. If improved → git commit
8. If no progress → try different angle, git reset
9. Update knowledge.md
10. Every 30 min: send progress to telegram
11. STOP ONLY when Agentic_Score >= 100 OR human stops
```

## Skills Improvement (ALWAYS do)
- Every night also improve 1 skill in ~/.hermes/skills/
- Target: SHS = 0 eventually
...
Tonight's focus: [Capability name]
Why chosen: [Reason based on Anh's needs]

Progress: [What was done]
Score: [Agentic_Score so far]

## Skills
- SHS: X (improved/unchanged)
...
The git push failed because there's no remote for the autoresearch repo (it's local-only). The knowledge is backed up in knowledge.md.
...
**Why chosen:** Highest impact for Anh's work. Better skills = better output for every task. Quick win tonight (pitfalls/examples are fast to add), foundational (improvements help ALL 16 capabilities).
...
## AI Agents Research
- Techniques documented: 0 (not focused tonight)

## Hermes Agentic
- **Capability worked on:** Skill Creation (#8)
- **Score so far:** +40 (skills_improved × 10)

## Git Commits
- `2f8bb54` — Skills improvement: added pitfalls to 4 skills
- `4d50d19` — Update RESULTS.tsv

## Next Run Recommendations
1. Improve more skills (github, terminal next)
2. Investigate self-heal vs lint ordering issue (461 broken links persist)
3. Archive old Telegram transcripts to reduce orphan pages
4. Explore Reflexio for potential Hermes self-improvement pattern
