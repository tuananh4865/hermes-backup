...
### ⚠️ CRITICAL: Worker Cron Jobs Use WRONG Prompt (2026-05-06)

**STATUS: ALL worker cron jobs misconfigured — they run `hermes-autoresearch` instead of worker-specific prompts.**

Evidence from 2026-05-06 morning scan:
- `ce3701b4dcdd` (Content Creator 8AM) → output shows autoresearch content
- `e4fb0c36e9f7` (Research Analyst 8:30AM) → output shows autoresearch content
- All worker `outputs/` directories: EMPTY

**This is the #1 reason workers aren't producing output.**

Each worker cron job needs its OWN specific prompt:
- Content Creator → TikTok scripts + Gen Z slang + trends
- Research Analyst → TikTok Shop products + competitor analysis
- Orchestrator → coordination + briefing format

**FIX:** Update each worker cron with `cronjob update --job_id {id} --prompt "..."` using worker-specific prompts from the Worker SOUL.md files, NOT `hermes-autoresearch`.
...
| Job ID | Name | Schedule | Worker | **Status** |
|--------|------|----------|--------|------------|
| ce3701b4dcdd | Content Creator Morning Brief | 0 8 * * * | content-creator | ⚠️ WRONG PROMPT |
| 50bc2c2dfbb3 | Content Creator Evening Report | 0 18 * * * | content-creator | ⚠️ WRONG PROMPT |
| e4fb0c36e9f7 | Research Analyst Morning Brief | 30 8 * * * | research-agent | ⚠️ WRONG PROMPT |
| 1c425ba42980 | Research Analyst Evening Report | 30 18 * * * | research-agent | ⚠️ WRONG PROMPT |
...
|--------|------|----------|---------|------------|
| 045a44210a59 | Orchestrator Morning Briefing | 0 9 * * * | Scan overnight crons, prepare priorities for Anh | ⚠️ WRONG PROMPT |
| f1584a9a1d86 | Orchestrator Agent Monitor | 0 */2 * * * | Check workers every 2h, nudge if stalled >2h | ⚠️ WRONG PROMPT |
| fc2191d508a3 | Orchestrator Nightly Consolidation | 0 21 * * * | Collect all agent outputs, compile learnings | ⚠️ WRONG PROMPT |

**All three orchestrator crons also run `hermes-autoresearch` instead of coordination-specific prompts.**
