...
def main():
    ...
    # 2. Check for câu treo
    print("")
    print("═══ Check 2: CÂU TREO ═══")
    treo = []
    conj_end = re.compile(r'\s(và|với|của|là|thì|nhưng|nên|rồi|mà|đó|như|vậy)$', re.IGNORECASE)
    for i, s in enumerate(segs[:-1]):
        text = s['text'].strip().rstrip('.,!?;:')
        if conj_end.search(text.lower()):
            treo.append((s['start'], text))
    if treo:
