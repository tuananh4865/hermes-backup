...
6. **Single-hypothesis anchor on "model bug" (NEW 2026-07-23)** — When output doesn't match expectation, agent's first guess is often "the model has a bug". Wrong 80%+ of the time. The actual cause is usually a config / input / prompt that the user knows better than you. **Test 3-4 variants A/B/C/D BEFORE conclude "model bug"**. Real case 23/07 OmniVoice: agent guessed "model always leaks ref text" → tested 4 variants of `ref_text` length (full 17s / 1 sent / 2 sent / minimal) → discovered "2 câu ~100 chars = no leak" was the fix. The bug was the PROMPT, not the model. Variant matrix saves hours of "model blame" loops. Companion rule: when user explicitly challenges your root cause ("không phải model bug, em test prompt lại đi"), **immediately do variant test** instead of defending the original hypothesis. See also `tts-voice-clone-test` skill § "Step 8 — Test variants before conclude" for 4-variant A/B/C/D matrix on prompt + config.
