...
## Recipe 12: Voice Template — 5 segments VỚI emotion tags (BẮT BUỘC)

**🔴 MANDATORY: Mỗi segment PHẢI có ≥1 emotion tag.** Voice phẳng = chưa đạt chuẩn voice thật (dùng cho TikTok, YouTube, podcast, v.v.).

### Template 5-segment (HOOK→PROBLEM→SOLUTION→USP→CTA)
...
model = OmniVoice.from_pretrained("k2-fsa/OmniVoice", device_map="mps", dtype=torch.float16)
prompt = VoiceClonePrompt.load("/Volumes/Storage-1/Hermes/voice-prompts/tuan_anh.pt")
gc = OmniVoiceGenerationConfig(pad_duration=0.0, fade_duration=0.0)  # NO PADDING

# HOOK — surprise + laughter (dừng scroll)
texts = [
    ("01_hook",    "[surprise-oh] Các bạn ơi, hôm nay mình giới thiệu một sản phẩm cực kỳ hot trên TikTok Shop[laughter]"),
    ("02_problem", "[sigh] Nhiều anh em phản hồi là máy hút bụi cũ quá ồn, pin yếu, lại còn nặng nữa. Đúng không?"),
    ("03_solution","Chiếc máy hút bụi này nhỏ gọn, nhẹ tay, pin sạc nhanh, lại còn siêu êm. Một lần sạc dùng được cả tuần."),
    ("04_usp",     "[question-ah] Điểm mình thích nhất là nó có ba đầu hút khác nhau, dùng được cho mọi ngóc ngách trong nhà. Giá thì chỉ hơn hai trăm nghìn thôi."),
    ("05_cta",     "[laughter] Anh em nào thích thì bấm giỏ hàng bên dưới đi nhé[confirmation-en] Freeship toàn quốc. Cảm ơn anh em đã xem."),
]

os.makedirs("batch_results", exist_ok=True)
for sid, text in texts:
    audio = model.generate(text=text, language="vi", voice_clone_prompt=prompt, generation_config=gc)[0]
    sf.write(f"batch_results/{sid}.wav", audio, model.sampling_rate)
```

### Emotion mapping chuẩn TikTok

| Segment | Tag(s) | Tại sao |
|---|---|---|
| HOOK | `[surprise-oh]` + `[laughter]` | Wow factor + engagement (dừng scroll) |
| PROBLEM | `[sigh]` | Empathy, chạm pain point |
| SOLUTION | (không tag, bình thường) | Authoritative, trình bày giải pháp |
| USP | `[question-ah]` | Engaging, viewer tự trả lời |
| CTA | `[confirmation-en]` + `[laughter]` | Friendly, call action |

### Verify emotion peak tăng

```bash
# Baseline (không tag): peak thường -3 to -4 dB
# Có emotion: peak -1 to -2.5 dB
ffmpeg -i batch_results/01_hook.wav -af volumedetect -vn -f null - 2>&1 | grep max_volume
# Expected: -1 to -3 dB (emotion tăng amplitude)
```

### Anti-pattern: KHÔNG BAO GIỜ skip emotion tags

```python
# ❌ SAI — voice phẳng, không engagement
audio = model.generate(text="Các bạn ơi, hôm nay giới thiệu sản phẩm hot.", ...)

# ✅ ĐÚNG — có emotion, voice thật hơn
audio = model.generate(text="[surprise-oh] Các bạn ơi, hôm nay giới thiệu sản phẩm hot[laughter]", ...)
```
