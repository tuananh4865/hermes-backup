...
Create Kanban tasks when any of those are true:
...
### 🔒 Tuấn Anh mandate (2026-06-18): Use Kanban for EVERY project, not just big ones

Tuấn Anh explicitly required (translated): "Every project current AND future, every item big AND small must have a plan + checklist + backlog so the user or other agents can audit." Override the "delegate_task for small one-shots" rule above when working with this user — always create a Kanban task with explicit checklist, even for tasks that look small. The audit trail and plan-with-checklist structure is the point, not the work itself.

**Mandatory for Tuấn Anh's projects:**
- Every project = its own Kanban board (`hermes kanban boards` → create board per project)
- Every task = `kanban_create` with `body` containing **explicit checklist** (see `templates/task-checklist.md`)
- Every action = logged via `kanban comment` or `kanban log` for audit
- Every phase transition = update task with `kanban_complete` summary

See `references/tuan-anh-workflow-mandate.md` for the full mandate transcript and gap analysis from 2026-06-18 session.
...
**Kanban DB corruption — recovery recipe.** SQLite kanban.db can corrupt on unclean shutdown (gateway crash, OOM, disk full). Symptom: `kanban` CLI hangs, "database is locked", or no board shows up. Recovery:
1. Check `~/.hermes/kanban.db.corrupt.*.bak-*` files — these are auto-rotated backups.
2. Run `hermes kanban init` (idempotent — creates fresh DB if missing, leaves existing alone).
3. If init fails: `rm ~/.hermes/kanban.db && hermes kanban init` (last resort — destroys in-flight tasks but keeps `.bak-*` files for forensics).
4. Verify with `hermes kanban boards` and `hermes kanban stats`. If `default` board reappears with `done=N` counts, recovery succeeded.
5. Discovered 2026-05-28 in production (multiple `.bak-*` files preserved). Root cause: macOS gateway restart while kanban dispatcher was mid-write. Mitigation: don't restart gateway mid-task; use `hermes kanban daemon stop` first.

**Task body must include checklist (Tuấn Anh mandate).** Tuấn Anh's 2026-06-18 mandate requires every task body to have an explicit checklist with checkboxes so he can see at a glance what's done vs pending. Don't write vague prose bodies — use the structured format from `templates/task-checklist.md`. Empty or prose-only bodies fail his audit requirement.

## Support files

- `templates/task-checklist.md` — copy-paste body format with Objective / Checklist / Deliverables / Dependencies / Verify / Status. Mandatory when working with Tuấn Anh; recommended for any user who needs to audit task progress.
- `references/tuan-anh-workflow-mandate.md` — 2026-06-18 session transcript + gap analysis explaining WHY this skill overrides its own "small one-shot = skip Kanban" rule for this user.
