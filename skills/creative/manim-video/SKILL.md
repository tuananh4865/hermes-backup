...
Run `scripts/setup.sh` to verify all dependencies. Requires: Python 3.10+ recommended (3.9 works with caveat — see Pitfall 2), Manim Community Edition v0.19-v0.20 (`pip install manim`), LaTeX (`texlive-full` on Linux, `mactex` on macOS), and ffmpeg. Reference docs tested against Manim CE v0.19.0 and v0.20.1.

### macOS install: pycairo requires pkg-config + cairo + libffi

`pip install manim` on macOS fails with `metadata-generation-failed` on pycairo if cairo headers aren't visible. Fix:

```bash
brew install pkg-config cairo
brew install libffi  # keg-only; cairo headers need PKG_CONFIG_PATH
...
# Python 3.9 venv ONLY needs: pip install importlib_metadata
# (v0.20+ on Python 3.10+ doesn't need this)
```

Verify: `python -c 'import manim; print(manim.__version__)'` should print `0.19.0` (or `0.20.1`).
...
## Pitfalls (v0.19 / v0.20-specific)

### Pitfall 1: `Polyline` doesn't exist in Manim v0.19+ — use `VMobject.set_points_as_corners`

Many older tutorials and StackOverflow answers use `from manim import Polyline`. This import FAILS silently in v0.19 (no error, just `NameError: name 'Polyline' is not defined` at runtime). Workaround:
...
# Or just use Polygon for closed shapes:
# polyline = Polygon(*pts, color=GREEN, stroke_width=1.5)  # connects last to first
```

For `isinstance(x, Polyline)` checks (e.g., to decide if a mobject is a polyline), add a flag attribute: `m._is_polyline = True` and check via `getattr(m, '_is_polyline', False)` — because `isinstance` arg must be a class, not a lambda.

### Pitfall 2: Python 3.9 venv needs `importlib_metadata` post-install

Manim v0.19 imports `from importlib_metadata import entry_points` at the plugin registry layer. On Python 3.9 (the macOS system Python default, not 3.10+), this module isn't in stdlib and breaks on `import manim`:

```python
ModuleNotFoundError: No module named 'importlib_metadata'
```

Fix: `pip install importlib_metadata` after `pip install manim`. Not needed on Python 3.10+ or when using Manim v0.20+ that migrated to `importlib.metadata`.

### Pitfall 3: -qm (720p30) is NOT "15-60s/scene" for multi-scene timelines

The Performance Targets table says `-qm` is 15-60s per scene. Reality: a 30s total timeline with 9 scenes at 720p30 renders in **~10 minutes**, not "15-60s/scene × 9 ≈ 2-9 minutes". This is because Manim re-encodes the entire scene timeline plus the final concat pass. Budget accordingly:

| Quality | 30s total (9 scenes) | 60s total |
|---|---|---|
| `-ql` | ~3 min | ~6 min |
| `-qm` | **~10 min** | ~20 min |
| `-qh` | ~30 min | ~1 hour |

**Practical implication:** Always ship `-ql` for video-compare tasks where resolution isn't critical. Save `-qm`/`-qh` for the final picked winner only. For background render: use `background=true` + `notify_on_complete=true` so the process doesn't block session timeout (foreground max 600s = 10 min — your `-qm` will be killed exactly at the wrong moment).

### Pitfall 4: `instanceof(mobject, SomeClass)` needs a real class, not a lambda

If you create mobjects via lambda or factory function and want to type-check them later:

```python
# WRONG — TypeError: isinstance() arg 2 must be a type
isinstance(m, lambda *args: ...)  # lambda is a function, not a class

# RIGHT — attach a flag attribute and check via getattr
def make_thing(*args):
    m = SomeRealClass(*args)
    m._is_thing = True
    return m

def is_thing(o):
    return getattr(o, '_is_thing', False)
```

This bites during animation logic that needs to filter specific mobject types.
