...
## Pitfalls (READ BEFORE PUSHING WORKFLOWS)

### 🚨 `.github/workflows/*.yml` rejected by `gh` OAuth — silent 404 trap
...
**Or via API:**
```bash
gh api repos/owner/repo/contents/.github/workflows/deploy.yml -X PUT ...
# {"message":"Not Found","status":"404"}     ← MISLEADING
```

**Root cause:** The `gh` CLI's default OAuth token has scopes `gist`, `read:org`, `repo` — **NOT `workflow`**. The `404` from the Contents API is GitHub deliberately refusing the request to write into `.github/workflows/` (it 404s instead of 403'ing so the path doesn't leak existence).

**Check your scopes:**
```bash
gh auth status
# Token scopes: 'gist', 'read:org', 'repo'   ← no 'workflow'
```

**Workarounds (pick one):**

1. **Don't create the workflow file via gh at all** — push workflow files BEFORE authorizing gh, or use a Personal Access Token with `workflow` scope in the git remote URL:
   ```bash
   git remote set-url origin https://x-access-token:ghp_xxx@github.com/owner/repo.git
   git push origin main
   ```

2. **Use `gh auth refresh`** — opens browser for OAuth flow (will TIME OUT in headless/non-interactive sessions — see [shipping-and-launch] for the GitHub Pages workaround that avoids this entirely).

3. **Bypass gh entirely** — use the GitHub UI to create the workflow file, then `git pull`.

4. **For GitHub Pages specifically:** you probably don't NEED a custom workflow — see the "Static Site Deployment" section in `shipping-and-launch`.

**Don't waste 7+ attempts debugging this** — check token scopes FIRST. Discovered by Tuấn Anh's session 2026-06-22 setting up mini-rpg-games.

### Other `gh api` silent 404s

- `gh api repos/X/contents/path/in/.github/` → 404 if folder doesn't exist (use `git/trees` endpoint or `mkdir -p` via direct git push)
- `gh api repos/X/git/trees` POST → 404 if base_tree invalid or token missing write scope
- Always `curl -sI` the final URL after API write to confirm 200/201, never trust 2xx from `gh api` blindly
...
- `references/github-code-review.md` — detailed review guide
