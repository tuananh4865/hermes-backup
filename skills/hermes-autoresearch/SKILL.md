...
**⚠️ CRITICAL (2026-06-25): MiniMax 1027 errors on brand-name queries**
`mcp_MiniMax_web_search` returns `1027-output new_sensitive` errors when the query contains brand names: DJI, GoPro, Sony, Insta360, Zhiyun, Insta360, Shopee, etc.
**Symptom:** 3 consecutive 1027 errors, then switch to exa
**Fix:** Use `mcp_exa_web_search_exa` for queries involving camera/gear brand names OR e-commerce platforms (Shopee, Lazada, Tiki). Reserve MiniMax for generic Vietnamese trend queries without brand/platform names.
**Verification (2026-06-25):** "DJI Osmo Action 4 Pro" → 1027 error. "TikTok Shop Vietnam trending camera" → success. "Shopee tripod mount accessory" → 1027 error (2026-06-26). Pattern: brand keywords + e-commerce platform names trigger the filter, generic queries don't.
**Shopee confirmed as brand-name trigger (2026-06-26):** MiniMax 1027 on "Shopee tripod mount accessory trending June 2026 deals commission affiliate" — switched to exa, succeeded. Always use exa for Shopee/Lazada/Tiki-related queries.

**⚠️ CRITICAL (2026-06-29): Exa MCP also unreachable**
Exa MCP failed 6 consecutive times on 2026-06-29. When BOTH MiniMax (1027 on brand) AND exa (unreachable) fail:
1. Web search is essentially blind for Shopee/camera brand content
2. Output confidence MUST be marked **LOW** regardless of how much research was attempted
3. Document the limitation explicitly in the output file
4. Flag every deal with "⚠️ Commission rate phải verify trên Seller Center trước khi push"
5. Consider pivoting to pre-vetted deal research (aggregator sites, KOL posts) instead of real-time platform data

Use `mcp_MiniMax_web_search` (more reliable than exa MCP) for generic queries. If 1027 error OR brand-name query → use `mcp_exa_web_search_exa` instead.
...
## Content Creator Cron Prompts — Quality Bar & Scope Discipline (2026-06-17 UPDATE)
...
### ⚠️ CRITICAL LIMITATION: Shopee Affiliate Dashboard Data (2026-06-17)

**Problem:** Shopee Affiliate Seller Center (commission rates, EPC, cookie window, CommissionsXtra offers) requires LOGIN — NOT accessible via web scraping/search.

**What IS accessible:**
- Shopee.vn product pages (prices, ratings, sales volume)
- Public commission rate ranges (2-6% base + up to 40% CommissionsXtra from published fee structures)
- KOL reviews on TikTok/YouTube
- User reviews on Shopee/e-commerce

**What is NOT accessible (requires seller login):**
...
**Research approach when Seller Center data is unavailable:**
1. Document product prices + ratings from Shopee.vn
2. Use publicly available commission rate RANGES (2-6% base category rate)
3. Flag every deal with: "⚠️ Commission rate MUST verify on Seller Center before pushing"
4. Recommend Anh to manually check Seller Center for final affiliate decision

**This limitation applies to ALL Shopee Affiliate research jobs.**
...
- `references/shopee-vietnam-scraping-blocked-2026-06-19.md` — **Shopee Vietnam blocked** (2026-06-19): mcp_exa_web_fetch_exa times out on Shopee.vn. Workaround: use web search snippets + third-party affiliate sites. Commission rates from BitBrowser/Cuelinks, exact rates require Seller Center login.
- `references/shopee-research-limitations-june-2026.md` — **Shopee research limitations** (2026-06-29): Both MiniMax (1027 on brand queries) AND exa (unreachable) failed simultaneously. Confidence MUST be LOW when both backends fail. Includes accessibility matrix (what IS vs is NOT accessible).
