---
title: AI Agent Articles — Research Log
created: 2026-06-01
updated: 2026-07-19
---

# AI Agent Articles — Research Log
...
## 2026-07-19: Cyril @cyrilXBT — "Self-Correcting AI Loop" (X Article via tweet 2077827005777588266)

- **URL:** https://x.com/cyrilXBT/status/2077827005777588266
- **Author:** Cyril @cyrilXBT (AI/Tech/Crypto educator, Obsidian + Claude Code stack)
- **Type:** Original essay (Linked X Article — "How to Build a Self-Correcting AI Loop That Catches Its Own Mistakes Before You See")
- **Engagement:** 23 replies, 65 retweets, 1.2M impressions, 1.1K bookmarks
- **Why loaded:** Anh shared URL 19/07 → asked for analysis

### Core Thesis
Architecture, not better prompting. Separated Builder-Judge-Manager roles so the Judge is not contaminated by the same blind spots that created the mistake. Judge MUST reference ground truth (source material / brief / test suite) — NOT just re-read Builder's output, otherwise confidently wrong answers sail past.

### 3 Roles + 3 Ground-Truth Sources
- **Builder** produces structured output (`draft` + `confidence` + `uncertainty_list`)
- **Judge** evaluates against source material / brief / test suite (per task type)
- **Manager** routes on per-check pass/fail (fact-check / brief-compliance / scope mismatch), hard cap 3 retries → human escalation

### Confidently-Wrong Test
Feed Judge an output you KNOW is subtly wrong. If Judge passes it → ground truth reference isn't actually being checked.

### Mapping to Hermes (70% match, 3 gaps)
1. **Builder uncertainty list** — append `uncertainty_list: [claim_X, claim_Y]` to structured outputs
2. **Confidence threshold routing** — if Judge confidence < 0.6 → escalate human immediately (currently missing in `adversarial_verify.py`)
3. **Confidently-wrong test pattern** — add `KNOWN_BAD_CASE` fixture to `references/test-fixtures/` for every verifier script

### Real Hermes Cases That Validate Cyril's Thesis
- **Yonex specs (07/08)** — adversarial verifier caught số liệu sai vì `yonex-specs-reference.md` (ground truth) was wired
- **ARMAF routing (07/08)** — verifier did NOT catch wrong-project routing vì project ground truth not wired
- **clip 0704, mascot Vui Vẻ V3, gateway path (12/07)** — verifier PASSED confidently-wrong outputs vì transcript/style/file-path ground truth not referenced
- Pattern: ground truth reference = make-or-break

### Action Items for Anh
1. Patch `~/.hermes/scripts/adversarial_verify.py` to add `< 0.6` confidence threshold + escalate-to-human routing
2. Wire `uncertainty_list` field through structured Builder outputs (~10 token cost per output)
3. Add `references/test-fixtures/` directory with KNOWN_BAD_CASE fixtures for each verifier script
4. Audit which Hermes verification scripts have ground-truth references (Yonex-style) vs which don't (clip 0704-style)

### Status: ANALYZED, FRAMEWORK 0 added to SKILL.md, full summary in `references/cyrilxbt-bjm-summary.md`
...
| **Builder-Judge-Manager Self-Correcting** | **Cyril @cyrilXBT (07/2026)** | **3 roles + ground-truth Judge + confidently-wrong test** | **Any pipeline that builds → ships → user review → fix → re-ships** |
...
5. **Ground-truth reference is make-or-break** (Cyril 07/2026) — Judge without source material only checks coherence, not correctness