...
gc = OmniVoiceGenerationConfig(pad_duration={pad_duration}, fade_duration={fade_duration})
...
'''
    ...
    if args.text:
        output = args.output
        code += f'''
audio = model.generate(text={json.dumps(args.text)}, language="vi", voice_clone_prompt=prompt, generation_config=gc)[0]
...
for i, s in enumerate(samples, 1):
    ...
    t0 = time.time()
    audio = model.generate(text=text, language=lang, voice_clone_prompt=prompt, generation_config=gc)[0]
    dt = time.time() - t0
