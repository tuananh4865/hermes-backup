...
## NEW: Chrome Keychain Cookies — Why Cookie Extraction Fails on macOS (2026-06-01)

**Session context:** Attempted to extract X.com cookies via `browser-harness CDP` and `computer_use` to enable automated X posting. Chrome was confirmed logged in (12 tabs open, x.com/home visible via osascript), but ALL cookie extraction methods returned empty results.

### The Core Problem

Chrome on macOS stores persistent session cookies (auth tokens, remember-me tokens) encrypted in the **macOS Keychain** — a system-level security store that sits below the application layer.

**What this means:**
- `browser-harness -c 'cdp("Network.getCookies", ...)'` → `[]` (empty)
- CDP `Network.getCookies` via any method → only returns non-Keychain session cookies
- `computer_use` driving Chrome UI → CAN interact with the rendered page, CANNOT read Keychain
- Playwright `context.cookies()` → empty unless user manually logged in first in that Playwright context
- Any automation tool claiming to "read Chrome cookies" on macOS → lying or using a workaround that requires prior manual login

**Root cause:** Chrome on macOS calls `SFKeychainApi` or similar system APIs to store cookies. The encryption key lives in the Secure Enclave/Keychain daemon (`securityd`), which is a separate process from Chrome. No other process can decrypt these values.

### Diagnosis Steps

```bash
# Step 1: Confirm Chrome IS logged in (this works)
osascript -e 'tell application "Google Chrome" to get URL of active tab of front window'
# → https://x.com/home  ✅ Chrome is logged in

# Step 2: Try browser-harness (this will fail)
browser-harness -c 'cdp("Network.getCookies", urls=["https://x.com"])'
# → [] or session-only  ❌ Keychain blocks access

# Step 3: Confirm this is the Keychain problem, not a Chrome session problem
# The fact that step 1 works but step 2 doesn't = Keychain encryption, not login state
```

### The Automation Path for X — Decision Tree

```
Anh muốn automate X?
│
├─ Dùng xurl (OAuth 2.0 API)? 
│   → Setup X Developer account → OAuth flow → xurl whoami → POST được
│   ✅ RECOMMENDED — official API, no cookies needed
│
├─ Extract cookies để dùng Playwright/requests?
│   → ❌ IMPOSSIBLE on macOS — Keychain blocks ALL tools
│   → Don't waste time trying
│   → Switch immediately to xurl approach
│
└─ Browser UI actions (click, type, scroll)?
    → computer_use + osascript ✅ WORKS for UI interaction
    → BUT can't auth as the user without OAuth
```

### Quick Verification Test for Future Sessions

Before spending time on cookie-based approaches, always verify the user's Chrome session state with osascript first:

```bash
osascript -e 'tell application "Google Chrome" to get URL of active tab of front window' 2>/dev/null || echo "Chrome not accessible"
```

If this returns a logged-in URL (x.com/home, etc.) but `browser-harness -c 'cdp("Network.getCookies", ...)'` returns empty → **Keychain problem, skip cookies, use xurl OAuth.**

### Historical Session Results (2026-06-01)

| Tool | Chrome Logged In? | Cookie Extraction? | Usable for X Post? |
|------|-------------------|-------------------|-------------------|
| osascript | ✅ | ❌ | ❌ (read only) |
| browser-harness CDP | ✅ | ❌ Empty (Keychain) | ❌ |
| computer_use | ✅ | ❌ (UI only) | ❌ (no auth) |
...
| Cookie extraction (any tool) | ❌ Keychain blocks all methods |