...
13. **PROJECT WORKFLOW v2.1 (2026-06-17) — 6-Step Loop Engine với RESEARCH BẮT BUỘC**: Cho projects > 2 tuần / > 3 phases / > 5 agents. Structure: `wiki/projects/{id}/{hub.md, phases/, research/, tasks/, actions/, decisions/, logs/}`. Loop = **RESEARCH → PLAN → RESEARCH → EXECUTE → VERIFY → NEXT** (6 bước). RESEARCH là BẮT BUỘC trước PLAN (Step 0, khi tạo project/phase mới) và trước EXECUTE (Step 1.5, khi có decision point lớn). Verify MUST pass trước khi next task. Auto-log mỗi session vào cả global log.md và per-project logs/. CI gate: `bash ~/.hermes/scripts/check-project-compliance.sh {project-id}` (8 checks, includes `research_refs` field requirement v2.1). Full spec: `~/.hermes/profiles/_shared/project-loop-engine.md`. User guide: `~/.hermes/docs/project-workflow-v2.md`. Skill: `project-workflow-v2`. Live example: `/wiki/projects/content-creator/`. **User feedback 17/06 10:55:** "research là kỹ năng bắt buộc trước plan và cũng bắt buộc trước execute" — v2.0 (4 bước) MISSED this mandate, v2.1 fixes.
...
**PITFALL 26 (2026-06-17): Self-Verification Bias — Always Route to Different Profile**

**User signal (Tuấn Anh 2026-06-17)**: "Nếu em tự check nó sẽ không còn khách quan nữa và có tỉ lệ cao bị tự nhận PASSED."

**Symptom**: Orchestrator (default) marks own work as PASS. Conflicts of interest reduce verification quality. Confirmation bias, sunk cost, ego all push toward self-PASS.

**Root cause**: Self-verification always fails objectivity. The agent wants to be done, doesn't want blame for FAIL, biases toward "looks good."

**Fix (MANDATORY — verified 2026-06-17 with 5-agent chain)**: ALWAYS route work to a DIFFERENT profile for verification. The separation of concerns:

| Maker (creates) | Checker 1 (style) | Checker 2 (security) | Checker 3 (function) |
|-----------------|-------------------|----------------------|----------------------|
| engineering-lead | code-reviewer | security-engineer | qa-agent |
| content-director | - | - | qa-agent |
| research-lead | - | - | qa-agent |
| memory-curator | - | - | qa-agent |
| operations-manager | - | - | qa-agent |

**The 4-stage chain (engineering-lead → code-reviewer → security-engineer → qa-agent)** was demonstrated end-to-end on 2026-06-17:
- engineering-lead: wrote /tmp/el-test/secure.py
- code-reviewer: APPROVED 9.0/10 (PEP 585 suggestion)
- security-engineer: SHIP_OK 9.0/10 (noted theoretical TOCTOU)
- qa-agent: PASS 9.5/10 (independently verified)

**Rule**: NEVER let default Orchestrator verify its own work. The qa-agent profile exists EXACTLY for this purpose. Set up the chain before shipping.

**Anti-patterns to avoid:**
- ❌ "Looks good to me" — vibes, not evidence
- ❌ Skipping qa-agent "to save time" — quality collapse
- ❌ Letting qa-agent also do the work — conflict of interest in reverse
- ❌ One agent doing ALL 3 checks (style + security + function) — single point of bias

**Companion skill**: `agentic-company-setup` — contains full SOUL.md template for qa-agent + 4-stage chain recipe.
...
## Periodic Routing Audit (operations-manager cron)

For 6h profile health audits (stuck tasks, pending QA, idle agents), see `references/profile-routing-audit.md` — workflow + classification thresholds + sample output format.
